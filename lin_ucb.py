...
class UCBLinearBandit:
    ...
    def __init__(self, n_bins, feature_dim, max_price, lambda_reg=LAMBDA, alpha=ALPHA):
        ...
        self.action_values = (edges[:-1] + edges[1:]) / 2
    ...
    def _uncertainty(self, i, phi):
        # compute alpha * sqrt(phi^T A_i^-1 phi) without explicitly inverting A
    ...
    def select_action(self, context_features):
        ...
        for i in range(self.n_bins):
            ...
            exploit = phi_i @ theta_i
            explore = self._uncertainty(i, phi_i)
...
def train():
    ...
    for ep in range(N_EPISODES):
        ...
        print(f"Ep {ep+1}/{N_EPISODES} | avg reward = {avg:.5f} | steps = {total_steps}")
    ...
    plt.title("UCB Linear Bandit - Dynamic Pricing")
    plt.legend()
